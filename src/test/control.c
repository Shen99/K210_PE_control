...
// 2023年10月1日 22:47:49 update
/*
% *****2023.9.25*****
% 1.MPC predict to K+2
% 2.Optimize the Udceq which is computed in voctor effect in L current slope
% 3.Consider to add the us_k1 and us_k2 to improve the accuracy
% 4.Optimize the current of each capacitor
%5.优化补偿时间的选取,只在合成向量1和4进行补偿,避免补偿对其它无关电容造成影响
*/
...
// 交流电流
double meas_ik = 0.1f; // 7606通道二》1》》》》交流电流
...
double Udceq_k1 = 0.2f; //等效直流电压
double meas_io1 = 0.1f; // 通道2》》》输出端电流
double meas_io2 = 0.1f; // 通道3 》》》输出端电流
...
void get_ik_ref(double us, double udc)
{
    ...
    // 闭环
    // ik_ref_calcu = abs_float(us / us_attitude * 0.2);
}
...
void MPCcontrol(double ik_ref, double Uco1, double Uco2, double Ucf1, double Ucf2, double us, double ik, double L, double Ts, double udc, double io)
{
    ...
    I_k1 = ik + Ts * VL_k1 / L; // K+1时刻的L电流
    // compute the K+1 time every C's voltage
    Icf1_k1 = I_k1 * (G2_last - G1_last) / Ts;
    Icf2_k1 = I_k1 * (G3_last - G4_last) / Ts;
    Ico1_k1 = I_k1 * (Ts - G2_last) / Ts;
    Ico2_k1 = I_k1 * (Ts - G3_last) / Ts;

    Ucf1_k1 = Icf1_k1 * Ts / Cf + Ucf1;
    Ucf2_k1 = Icf2_k1 * Ts / Cf + Ucf2;
    Uco1_k1 = Ico1_k1 * Ts / Co + Uco1;
    Uco2_k1 = Ico2_k1 * Ts / Co + Uco2; // 待优化
    // 2023.9.25引入等效直流侧电容电压Udceq_k1，在的一定程度上优化电容电压偏离目标值时
    // 用直流电压不准确，可能会选错电流矢量的问题
    Udceq_k1 = (Ucf1_k1 + Ucf2_k1 + Uco1_k1 + Uco2_k1) * 2 / 3;
    // 每一组矢量在电感电流上产生的斜率
    ...
    show[10] = com_v_number ;
    show[11] = t_balance_cf/ control_Ts;
    show[12] = t_balance_co/ control_Ts;
    show[13] = t_ref_cf/ control_Ts;
...
int control_loop(void *ctx)
{
    ...
    */
    ...
    meas_us = adc_buf[1] * 0.1139f - 0.029f; // 7606通道2》》》》交流电压
    // 茶花传感器系数 meas_ik = 0.0051f*adc_buf[0]-0.1995f;                              //7606通道一》》》》交流电流
    //
    meas_ik = 0.0049 * adc_buf[0] - 8.0961; // 7606通道一》》》》交流电流(AMC1301)
    ...
    meas_io1 = 0.0049 * adc_buf[2] - 8.0961;  // 7606通道2  待添加 需校准
    meas_io2 = 0.0049 * adc_buf[3] - 8.0961;  // 7606通道2  待添加 需校准
    //

    meas_udc = meas_u1 + meas_u2; // 直流输出电压

    if (meas_udc > udc_ref/4 )//(meas_u1 > udc_ref / 4 && meas_u2 > udc_ref / 4)
    {
        // MPC out put
        ...
        // output PWM 0-1-2-3
        // MPC计算与输出
        MPCcontrol(ik_ref_calcu, meas_u1, meas_u2, meas_u3, meas_u4, meas_us, meas_ik_abs, L_value, control_Ts, meas_udc, meas_io1);
    }
    else
    {
        PWM_stop();
